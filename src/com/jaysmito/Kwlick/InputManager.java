...
public class InputManager{
	...
	protected InputManager(){
		keys = new boolean[65535]; // I know this is inefficient but it is the easiest way to do it!
		for(int i=0;i<keys.length;i++)
			keys[i]=false;
	}

	protected void setKey(int key, boolean state){
		try{
			keys[key] = state;
		}catch(Exception ex){}
	}
	...
	public int GetAxis(String axis){
		switch(axis.toLowerCase()){
			...
			case "jump":{
				return keys[VK_SPACE]?1:0;
			}
			case "crouch":{
				return (keys[VK_C] || keys[VK_CONTROL])?1:0;
			}
			default :{
				return 0;
			}
